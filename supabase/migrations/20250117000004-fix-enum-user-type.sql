-- Migration pour corriger le problème ENUM user_type
-- Date: 17 janvier 2025

-- 1. Vérifier et supprimer le type ENUM user_type s'il existe
DROP TYPE IF EXISTS user_type CASCADE;

-- 2. Créer le type ENUM user_type avec les bonnes valeurs
CREATE TYPE user_type AS ENUM ('candidate', 'company', 'admin');

-- 3. Modifier la colonne user_type pour utiliser le type ENUM
ALTER TABLE public.profiles 
ALTER COLUMN user_type TYPE user_type 
USING user_type::user_type;

-- 4. Supprimer l'ancienne contrainte CHECK
ALTER TABLE public.profiles 
DROP CONSTRAINT IF EXISTS profiles_user_type_check;

-- 5. Recréer la fonction handle_new_user avec le bon cast
...
-- 6. Recréer la fonction handle_user_update avec le bon cast
...
-- 8. Synchroniser les utilisateurs existants avec le bon cast
INSERT INTO public.profiles (
  user_id,
  email,
  first_name,
...
SELECT 
  ...
  COALESCE(au.raw_user_meta_data->>'user_type', 'candidate')::user_type,
  true,
...
-- 9. Créer des politiques RLS pour les administrateurs
